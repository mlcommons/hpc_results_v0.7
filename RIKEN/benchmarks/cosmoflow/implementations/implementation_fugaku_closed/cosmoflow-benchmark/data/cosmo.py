...
def _parse_data(sample_proto, shape, apply_log=False, seed=None, do_augmentation=False, dist=None):
...
def construct_dataset(file_dir, n_samples, batch_size, n_epochs,
    ...
    def data_fn():
        ...
        # Parse TFRecords
        parse_data = partial(_parse_data, shape=sample_shape, apply_log=apply_log, seed=seed, do_augmentation=do_augmentation, dist=dist)
        def dataset_fn(ds):
            return tf.data.TFRecordDataset(ds, num_parallel_reads=4)
        data = data.apply(dataset_fn).map(parse_data, num_parallel_calls=4).cache()
        if shuffle:
...
def get_datasets(data_dir, sample_shape, n_train, n_valid,
    ...
    train_dataset_args = dict(batch_size=batch_size, n_epochs=n_epochs,
                              ...
                              apply_log=apply_log, prefetch=prefetch, dist=dist)
