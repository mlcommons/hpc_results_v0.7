    ...
    def __init__(self, filename, benchmark, organization, submission_log=True, comm_size=None):
        ...
        if submission_log:
            ...
            self.log_event(key = constants.SUBMISSION_PLATFORM,
                           value = f'abci_{self.comm_size}xV100_pytorch')
    
            self.log_event(key = constants.SUBMISSION_POC_NAME,
                           value = 'Koichi Shirahata')
    
            self.log_event(key = constants.SUBMISSION_POC_EMAIL,
                           value = 'k.shirahata@fujitsu.com')
