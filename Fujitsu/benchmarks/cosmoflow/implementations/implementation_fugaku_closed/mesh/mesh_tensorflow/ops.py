...
def mesh_init(axis_d_block, axis_h_block):
   global model_rank
   global model_size
   ...
   assert (world_size % (horizontal_size * vertical_size)) == 0
...
def allreduce_pyfunc(*x):
    #tf.logging.info("allreduce_pyfunc start")
    sendbuf = None
    size_array = []
    for t in x:
        buf = t.numpy()
        buf = np.ravel(buf)
        size_array.append(buf.size)
        if sendbuf is None:
            sendbuf = buf
            continue
        sendbuf = np.concatenate([sendbuf,buf],0)
    recvbuf = np.empty(sendbuf.shape, dtype=x[0].dtype.as_numpy_dtype)
    MPI.COMM_WORLD.Allreduce(sendbuf, recvbuf, MPI.SUM) # hybrid parallel
    Tensors = []
    s = 0
    for i in xrange(len(size_array)):
        Tensors.append(tf.convert_to_tensor(recvbuf[s:(s+size_array[i])].reshape(x[i].shape), dtype=x[i].dtype))
        s += size_array[i]
    #tf.logging.info("allreduce_pyfunc end")
    return Tensors
...
def tfallreduce(x):
    num_tensor = len(x)
    xl = list(x)
    #tf.logging.info("NUM_TENSORS: %s" %num_tensor)
    #tf.logging.info("TFALLREDUCE_INPUT_T: %s" %t)
...
def halo_exchange_with_MPI(x, blocks_dim, block_size_dim, halo_size, wrap=True):
  ...
  if partial_size > 0:
    ...
    #    [shift(left_margin, -(num_complete_blocks + 1), blocks_dim, wrap)])
    parts = (parts +
        [shift(after_sendrecv, num_complete_blocks + 1, blocks_dim, wrap)])
