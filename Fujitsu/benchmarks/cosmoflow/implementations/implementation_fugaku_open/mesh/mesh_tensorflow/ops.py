...
model_rank = 0
model_size = 1
model_parallel_comm = None
horizontal_comm = None
horizontal_size = 1
vertical_comm = None
vertical_size = 1
num_active_size = 1

def mesh_init(axis_d_block, axis_h_block, num_active_proc=world_size):
   global model_rank
   global model_size
   global model_parallel_comm
   global horizontal_comm
   global horizontal_size
   global vertical_comm
   global vertical_size
   global num_active_size
   num_active_size = num_active_proc
   horizontal_size = axis_h_block
   vertical_size = axis_d_block
   mpi_color = world_rank // (vertical_size * horizontal_size)
   mpi_key = world_rank % (vertical_size * horizontal_size)
   model_parallel_comm = MPI.COMM_WORLD.Split(mpi_color, mpi_key)
   model_rank = model_parallel_comm.Get_rank()
   model_size = model_parallel_comm.Get_size()
   
   horizontal_color = (world_rank // horizontal_size) + model_size
   horizontal_key = world_rank % horizontal_size
   horizontal_comm = MPI.COMM_WORLD.Split(horizontal_color, horizontal_key)
   
   vertical_color = (horizontal_key + mpi_color * horizontal_size) + model_size + horizontal_comm.Get_size()
   vertical_key = model_rank // horizontal_size
   vertical_comm = MPI.COMM_WORLD.Split(vertical_color, vertical_key)
   #MPI.COMM_WORLD.Barrier()
   assert (num_active_size % (horizontal_size * vertical_size)) == 0
   #assert (axis_d_block % 2) == 0
   #assert (axis_h_block % 2) == 0
...
class Lowering(object):
  ...
  def __init__(self, graph, mesh_to_impl, autostack=True, log_file=None):
    ...
    for op in graph.operations:
      #logger.info("Lowering operation %s" % op.to_string)
...
class LazyAllreduceSum(object):
  ...
  def __init__(self,
    ...
    self._reduced = None
    logger.info("Called LazyAllreduceSum")
...
class GenericGradOperation(Operation):
  ...
  def __init__(self, forward_op, grad_ys, name=None):
    # logger.info("forward inp %s, operations %s, grad_ys: %s",
...
def add_counter(val):
    global tag_counter
    tag_counter += val
    limit_tag_number = 3000
    if tag_counter > limit_tag_number:
        tag_counter -= limit_tag_number

def sendrecv_pyfunc(x,axis):
    if num_active_size <= world_rank:
        return x
    #logger.info("SendRecv_Pyfunc start")
    if axis == h_axis_idx:
        # horizontal communication
        rank = horizontal_comm.Get_rank()
        size = horizontal_comm.Get_size()
        if size == 1:
           y = tf.zeros(shape=x.get_shape(),dtype=tf.float32)
           return y
        if rank != 0:
           dest_peer = rank - 1
           req = horizontal_comm.isend(x,dest=dest_peer, tag=get_counter())
        if rank != (size - 1):
           recv_peer = rank + 1
           y = horizontal_comm.recv(source=recv_peer, tag=get_counter())
        else:
           y = tf.zeros(shape=x.get_shape(),dtype=tf.float32)
        if rank!= 0:
           req.wait()
        ...
        rank = vertical_comm.Get_rank()
        size = vertical_comm.Get_size()
        if size == 1:
           y = tf.zeros(shape=x.get_shape(),dtype=tf.float32)
           return y
        if rank != 0:
           dest_peer = rank - 1
           req = vertical_comm.isend(x,dest=dest_peer, tag=get_counter())
        if rank != (size - 1):
           recv_peer = rank + 1
           y = vertical_comm.recv(source=recv_peer, tag=get_counter())
        else:
           y = tf.zeros(shape=x.get_shape(),dtype=tf.float32)
        if rank!= 0:
           req.wait()
...
def sendrecv_backprop_pyfunc(x,axis):
    if num_active_size <= world_rank:
        return x
    #logger.info("SendRecv_Pyfunc start")
    if axis == h_axis_idx:
...
def simple_sleep(x):
    logger.info("sleeping start")
    time.sleep(1)
    logger.info("sleeping end")
    return x

@tf.function
def tfsendrecv_backprop(dy,axis):
    dx = tf.py_function(func=sendrecv_backprop_pyfunc, inp=[dy,axis], Tout=tf.float32, name="sendrecv_backprop")
    dx.set_shape(dy.get_shape())
    return dx

@tf.function
def tfsendrecv(x,axis):
    y = tf.py_function(func=sendrecv_pyfunc, inp=[x,axis], Tout=tf.float32, name="sendrecv")
    y.set_shape(x.get_shape())
    return y

@tf.function
def tfsleep(x):
    y = tf.py_function(func=simple_sleep, inp=[x], Tout=tf.float32, name="sleep")
    y.set_shape(x.get_shape())
    return y

# kasagi
class SendRecvOperation(Operation):

  def __init__(self, x, sendrecv_dim_name, name=None):
    super(SendRecvOperation, self).__init__([x], name=name or "sendrecv")
    dim_names = [dim.name for dim in x.shape.dims]
    if sendrecv_dim_name not in dim_names:
      raise ValueError("x does not contain a dimension named dim_name")
    self._axis = dim_names.index(sendrecv_dim_name)
    self._outputs = [Tensor(self, x.shape, x.dtype)]

  def gradient(self, grad_ys):
    dy = grad_ys[0]
    return [sendrecv_backprop(dy, self._axis)]

  def lower(self, lowering):
    mesh_impl = lowering.mesh_impl(self)
    inputs = self._inputs[0]
    logger.info("SendRecvOperation inputs %s" %(inputs))
    def communication_fn(x, axis):
       logger.info("SENDRECV_TENSOR before %s" %(x))
       x = tfsendrecv(x,axis)
       logger.info("SENDRECV_TENSOR after %s" %(x))
       return x
    y = mesh_impl.slicewise(communication_fn, lowering.tensors[inputs], self._axis)
    lowering.set_tensor_lowering(self.outputs[0], y)

def sendrecv(x, sendrecv_dim_name, name=None):
   return SendRecvOperation(x, sendrecv_dim_name, name).outputs[0]

class SendRecvBackPropOperation(Operation):

  def __init__(self, x, axis, name=None):
    super(SendRecvBackPropOperation, self).__init__([x], name=name or "sendrecv_backprop")
    self._axis = axis
    self._outputs = [Tensor(self, x.shape, x.dtype)]
  ...
  def lower(self, lowering):
    mesh_impl = lowering.mesh_impl(self)
    inputs = self._inputs[0]
    logger.info("SendRecvBackPropOperation inputs %s" %(inputs))
    def communication_fn(x, axis):
       logger.info("SENDRECV_BACKPROP_TENSOR before %s" %(x))
       x = tfsendrecv_backprop(x,axis)
       logger.info("SENDRECV_BACKPROP_TENSOR after %s" %(x))
       return x
    dx = mesh_impl.slicewise(communication_fn, lowering.tensors[inputs], self._axis)
    lowering.set_tensor_lowering(self.outputs[0], dx)

def sendrecv_backprop(x, axis, name=None):
  return SendRecvBackPropOperation(x, axis, name).outputs[0]

def my_allgather(x,comm):
    send_requests = []
    recv_requests = []
    tensors = []
    if num_active_size <= world_rank:
        ...
        return tensors
    rank = comm.Get_rank()
    for dst in xrange(comm.Get_size()):
        send_requests.append(comm.isend(x,dest=dst,tag=get_counter()+rank))
    for rcv in xrange(comm.Get_size()):
        tensors.append(comm.recv(source=rcv, tag=get_counter()+rcv))
    for req in send_requests:
        req.wait()
    add_counter(comm.Get_size())
    return tensors

def allgather_pyfunc(x,axis):
    if axis == d_axis_idx:
        y = my_allgather(x, vertical_comm)
    else:
        y = my_allgather(x, horizontal_comm)
    return y

@tf.function
def tfallgather(x,axis):
    if h_axis_idx == axis:
        y = tf.py_function(func=allgather_pyfunc, inp=[x,axis], Tout=[tf.float32]*horizontal_size, name="allgather")
    if d_axis_idx == axis:
        y = tf.py_function(func=allgather_pyfunc, inp=[x,axis], Tout=[tf.float32]*vertical_size, name="allgather")
    z = []
    for t in y:
        t.set_shape(x.get_shape())
        z.append(t)
    return tuple(z)

class AllgatherOperation(Operation):
  def __init__(self, x, allgather_dim_name, name=None):
    super(AllgatherOperation, self).__init__([x], name=name or "allgather")
    dim_names = [dim.name for dim in x.shape.dims]
    if allgather_dim_name not in dim_names:
      raise ValueError("x does not contain a dimension named dim_name")
    self._axis = dim_names.index(allgather_dim_name)
    if self._axis == d_axis_idx:
       output_tensors = [Tensor(self, x.shape, x.dtype) for i in xrange(vertical_size)]
       self._MPIrank = vertical_comm.Get_rank()
       self._MPIsize = vertical_comm.Get_size()
    if self._axis == h_axis_idx:
       output_tensors = [Tensor(self, x.shape, x.dtype) for i in xrange(horizontal_size)]
       self._MPIrank = horizontal_comm.Get_rank()
       self._MPIsize = horizontal_comm.Get_size()
    self._outputs = output_tensors
       
  def gradient(self, grad_ys):
    dy = grad_ys[self._MPIrank]
    return [dy]
  ...
  def lower(self, lowering):
    ...
    inputs = self._inputs[0]
    def communication_fn(x, axis):
       y = tfallgather(x,axis)
       return y
    values = mesh_impl.slicewise(communication_fn, lowering.tensors[inputs], self._axis)
    for t, v in zip(self._outputs, values):
       lowering.set_tensor_lowering(t, v)

def allgather(x, axis, name=None):
  return AllgatherOperation(x, axis, name).outputs

def allreduce_pyfunc(*x):
    Tensors = []
    if num_active_size <= world_rank:
       np_list = [np.frombuffer(t, dtype=np.float32) for t in x]
       sample_buf = np.concatenate(np_list)
       send_buf = np.zeros(sample_buf.size, dtype=t.dtype.as_numpy_dtype)
       recv_buf = np.empty(send_buf.size, dtype=x[0].dtype.as_numpy_dtype)
       MPI.COMM_WORLD.Allreduce(send_buf, recv_buf, MPI.SUM) # hybrid parallel
       s = 0
       for i in range(len(x)):
           Tensors.append(tf.convert_to_tensor(np.reshape(np.frombuffer(recv_buf, dtype=np.float32, count=np_list[i].size, offset=s*4),x[i].shape),dtype=tf.float32))
           s += np_list[i].size
    else:
       np_list = [np.frombuffer(t, dtype=np.float32) for t in x]
       send_buf = np.concatenate(np_list)
       recv_buf = np.empty(send_buf.size, dtype=x[0].dtype.as_numpy_dtype)
       MPI.COMM_WORLD.Allreduce(send_buf, recv_buf, MPI.SUM) # hybrid parallel
       s = 0
       for i in range(len(x)):
...
"""
    sendbuf = x.numpy()
    recvbuf = np.empty(sendbuf.shape, dtype=x.dtype.as_numpy_dtype)
    MPI.COMM_WORLD.Allreduce(sendbuf, recvbuf, MPI.SUM) # hybrid parallel
    y = tf.convert_to_tensor(recvbuf,dtype=x.dtype)
    return y
"""
...
def tfallreduce(x):
    num_tensor = len(x)
    xl = list(x)
    y = tf.py_function(func=allreduce_pyfunc, inp=xl, Tout=[tf.float32]*num_tensor, name="allreduce")
    for i in xrange(num_tensor):
        y[i].set_shape(xl[i].get_shape())
    return tuple(y)

"""
    z = []
    for t in x:
        #logger.info("TFALLREDUCE_INPUT_T: %s" %t)
        y = tf.py_function(func=allreduce_pyfunc, inp=[t], Tout=tf.float32, name="allreduce")
        y.set_shape(t.get_shape())
        z.append(y)
    return tuple(z)
"""
def barrier_pyfunc(*x):
    MPI.COMM_WORLD.Barrier()
    return x
...
def tfbarrier(x):
    num_tensor = len(x)
    xl = list(x)
    y = tf.py_function(func=barrier_pyfunc, inp=xl, Tout=[tf.float32]*num_tensor, name="barrier")
    for i in xrange(num_tensor):
        y[i].set_shape(xl[i].get_shape())
    return tuple(x)
...
class AllreduceOperation(Operation):
  def __init__(self, xs, name=None):
    super(AllreduceOperation, self).__init__(xs, name=name or "allreduce")
    output_tensors = [Tensor(self, x.shape, x.dtype) for x in xs]
    self._outputs = output_tensors
       
  def lower(self, lowering):
    mesh_impl = lowering.mesh_impl(self)
    def communication_fn(*args):
       ys = tfallreduce(args)
       return ys
    values = mesh_impl.slicewise(communication_fn, *[lowering.tensors[x] for x in self._inputs])
    for t, v in zip(self._outputs, values):
       lowering.set_tensor_lowering(t, v)

def allreduce(x, name=None):
    return AllreduceOperation(x).outputs
...
def pool_fn(pool_fn_string):
  ...
  def max_pool3d_fn(x, ksize, strides, padding):
    logger.info("MAX_POOL_TENSOR %s" %(x))
    logger.info("MAX_PPOOLTENSOR_DIMS %s" %(x.shape.ndims))
    #x = tfsleep(x)
    return _tf_restore_batch_dims(
...
class MtfCheckpointSaverListener(tf.estimator.CheckpointSaverListener):
  ...
  def before_save(self, session, global_step_value):
    ...
    logger.info("About to write a checkpoint")

  def after_save(self, session, global_step_value):
    logger.info("Done writing checkpoint.")
...
class MtfRestoreHook(tf.estimator.SessionRunHook):
  ...
  def after_create_session(self, session, coord):
    logger.info("Before copy master to slices.")
    session.run(self._op)
    logger.info("Done with copy master to slices.")
...
def conv3d_to_dense(x, block_size_dim):
  if world_size == 1:
    return x
  axis_name = block_size_dim.name
  tensors =  allgather(x, axis_name)
  logger.info("C2D: %s" %tensors)
  return concat(tensors, block_size_dim.name)
...
def halo_exchange_with_MPI(x, blocks_dim, block_size_dim, halo_size, wrap=True):
  ...
  if partial_size > 0:
    left_margin = mtf_slice(x, 0, partial_size, block_size_dim.name)
    after_sendrecv = sendrecv(left_margin, block_size_dim.name)
    #parts = ( parts +
    #    [shift(left_margin, -(num_complete_blocks + 1), blocks_dim, wrap)])
    parts = (parts +
        [after_sendrecv])
  logger.info("HALO_EXCHANGE: %s" %parts)
  return concat(parts, block_size_dim.name)

def halo_exchange_ymzk(x, blocks_dim, block_size_dim, halo_size, wrap=True):
  """Concat each block with the margins of adjacent blocks.
  ...
  Args:
    ...
    blocks_dim: a Dimension in x.shape
    block_size_dim: a Dimension in x.shape
    halo_size: integer list [left, right]
    wrap: a boolean
  ...
  Returns:
    ...
    size is increased by 2*halo_size.
  """
  assert len(halo_size)==2
  if halo_size == [0, 0]:
    return x

  block_size = block_size_dim.size
  partial_size0 = halo_size[0] % block_size
  partial_size1 = halo_size[1] % block_size
  num_complete_blocks0 = halo_size[0] // block_size
  num_complete_blocks1 = halo_size[1] // block_size
  parts = [x]

  for i in xrange(1, num_complete_blocks0 + 1):
    parts = ([shift(x, i, blocks_dim, wrap)] + parts)
  logger.info("#parts1#, %s " %(parts))
  for i in xrange(1, num_complete_blocks1 + 1):
    parts = (parts + [shift(x, -i, blocks_dim, wrap)])
  logger.info("#parts2#, %s " %(parts))
  if partial_size0 > 0:
    left_margin = mtf_slice(x, 0, partial_size0, block_size_dim.name)
    parts = (parts +
        [shift(left_margin, -(num_complete_blocks0 + 1), blocks_dim, wrap)])
    logger.info("#parts3#, %s " %(parts))
  if partial_size1 > 0:
    right_margin = mtf_slice(
        x, block_size_dim.size - partial_size1, partial_size1,
        block_size_dim.name)
    logger.info("#x_type#, %s " %(x))
    logger.info("#right_margin_type#, %s " %(right_margin))
    #logger.info("#beforeSendRecv#, %s " %(right_margin))
    after_sendrecv = sendrecv(right_margin, block_size_dim.name)
    #logger.info("#afterSendRecv#, %s " %(right_margin))
    parts = (parts +
        [shift(after_sendrecv, num_complete_blocks1 + 1, blocks_dim, wrap)])
    logger.info("#parts4#, %s " %(parts))
  #print("#parts#",partial_size0, partial_size1, parts)
  return concat(parts, block_size_dim.name)
